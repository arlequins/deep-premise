...
Compress-Archive -Path dist/conversation/* -DestinationPath dist/Unseen-Order-0.2.0-Windows-x64.zip -Force
Get-FileHash dist/Unseen-Order-0.2.0-Windows-x64.zip,dist/conversation/Unseen-Order.exe,dist/conversation/Unseen-Order.pck -Algorithm SHA256 |
    ForEach-Object { $_.Hash.ToLowerInvariant() + '  ' + (Split-Path $_.Path -Leaf) } | Set-Content -Encoding ascii dist/SHA256SUMS.txt
Write-Host 'Portable package: dist/Unseen-Order-0.2.0-Windows-x64.zip'
